...
> [!IMPORTANT]
> **QUY TẮC KHỞI ĐẦU: Quy Trình Khảo Sát Tương Tác (Interactive Briefing Workflow)**
> Để đảm bảo hình ảnh quảng cáo được thiết kế cực kỳ ấn tượng, bám sát nhu cầu và không bị lệch tông, **Agent KHÔNG ĐƯỢC TỰ Ý sinh ảnh ngay lập tức**. Thay vào đó, hãy thực hiện khảo sát nhanh qua 4 bước tương tác dưới đây:
> 
> 1. **Hỏi khách hàng 4 câu hỏi định hình (Briefing Questions)**:
>    * **Bước 1 (USP)**: Đặc điểm/công dụng nổi bật nhất của sản phẩm cần nhấn mạnh là gì?
>    * **Bước 2 (Ad Angle)**: Góc tiếp cận quảng cáo mong muốn (Ví dụ: So sánh trước/sau, chỉ dẫn tính năng, đánh giá chân thực từ khách hàng, hay banner khuyến mãi)?
>    * **Bước 3 (Style & Brand Kit)**: Phong cách mỹ thuật (isometric, minimalist, cinematic) và bảng màu chủ đạo (nếu có mã màu cụ thể)?
>    * **Bước 4 (Style Reference)**: Khách hàng có ảnh mẫu hay ảnh phong cách/layout nào mong muốn không? (Nếu có, yêu cầu họ cung cấp tệp/đường dẫn để truyền vào tham số `--styleImageUrl` nhằm giúp AI bám sát phong cách thiết kế đó).
> 
> 2. **Xác nhận prompt thiết kế**:
>    * Sau khi nhận câu trả lời, hãy tóm tắt ý tưởng bố cục hình ảnh và trình bày prompt mẫu cho khách hàng xem trước trước khi chạy lệnh.
>    * Nếu khách hàng có ảnh mẫu, hãy truyền đường dẫn ảnh đó vào tham số `--styleImageUrl` khi gọi CLI:
>      ```bash
>      node vidtory-cli.js generate-image --prompt "..." --styleImageUrl "./sample_style.png" ...
>      ```
> 
